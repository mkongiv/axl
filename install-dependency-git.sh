...
## Filename: install-dependency-git.sh
...
gitsource="https://github.com/kongm/"
gitcmd="git clone "
extra_args=""
if [ $# -eq 2 ]; then
  extra_args=$2
...
if [ $1 == "piplib-gmp" ]; then
  ...
  pass=1
  ballext=gz
  url=$gitsource"piplib-gmp-mdt.git"
  tgtdir=$dependency
  usegit=1
fi
...
if [ $1 == "ponos" ]; then
  ...
  pass=1
  ballext=gz
  url=$gitsource"ponos-performance-lexicon.git"
  tgtdir=$dependency
  usegit=0
fi
...
if [ $usegit -eq 1 ]; then
  $gitcmd $url $tgtdir
  cd $tgtdir
  if [ $dependency == "ponos" ]; then
    git checkout ponos-axl
  fi
  if [ ! -f configure ]; then
    ./bootstrap.sh
  fi
else
  # Use standard tarballs
  if [ $ballext == "gz" ]; then
    tar_args=xvzf
  fi
  if [ $ballext == "xz" ]; then
